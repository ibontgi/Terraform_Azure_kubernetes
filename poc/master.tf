...
resource "azurerm_linux_virtual_machine" "azure_master" {
    name                            = "node.master"
    ...
    boot_diagnostics {
        storage_account_uri = azurerm_storage_account.staccount1cp2.primary_blob_endpoint
    }
    ...
    provisioner "file" {
        ...
        connection {
            type        = "ssh"
            user        = var.admin_user
            host        = azurerm_public_ip.myPublicIP1.ip_address
    ...
    provisioner "remote-exec" {
        inline   = [
            ...
            "sudo su - ansible -c 'ssh -t -q -o \"StrictHostKeyChecking no\" ${var.ansible_user}@${azurerm_network_interface.myNic1.private_ip_address} \"exit\"'",
            "echo  \"--------------------WORKER----------------------\"",
            ...
            "sudo su - ansible -c 'ssh -t -q -o \"StrictHostKeyChecking no\" ${var.ansible_user}@${azurerm_network_interface.myNic2.private_ip_address} \"exit\"'",
            "echo  \"--------------------NFS----------------------\"",
            "sudo sshpass -p \"${var.ansible_user_pass}\" ssh-copy-id -o \"StrictHostKeyChecking no\" -i /home/${var.ansible_user}/.ssh/id_rsa.pub ${var.ansible_user}@${azurerm_network_interface.myNic1.private_ip_address}",
            "sudo su - ansible -c 'ssh -t -q -o \"StrictHostKeyChecking no\" ${var.ansible_user}@${azurerm_network_interface.myNic1.private_ip_address} \"exit\"'",
            "echo  \"--------------------CHECK-----------------------\"",
    ...
    depends_on = [azurerm_linux_virtual_machine.azure_worker]
    tags = {
        environment = "PC2"
        host = "Master"
    }
